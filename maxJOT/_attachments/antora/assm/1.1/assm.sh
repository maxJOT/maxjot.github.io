...
# Verify Bash is version 4.2 or later (unicode support & -t 0.1).
#
if (( BASH_VERSINFO[0] < 4 || \
    (BASH_VERSINFO[0] == 4 && BASH_VERSINFO[1] < 2) )); then
  err1 ${LINENO} "Bash version 4.2 or later required." # read -t 0.1
  echo; exit 1
fi
...
function open_browser { #####################################################
  ...
  case ${BROWSER_LAUNCH} in
    ...
    ask) box r box_top
         ...
         box r "Offline content, history and cookies however remain."
         box r box_bottom
         echo
         get_reply \
