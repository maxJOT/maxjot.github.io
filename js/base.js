...
function adjustLink(linkEl) {
  ...
  // Skip SPA rewriting for anything in the "external" folder
  if (href && href.includes('/external/')) {
      linkEl.target = "_blank";   // open in new tab
      linkEl.rel = "noopener";    // security/performance
      return;                     // skip SPA hash rewriting
  }

  // Apply SPA logic for other internal links
  ...
  // Optional: external links (any host not equal to current)
  if (href && linkEl.host !== location.host) {
